...
var TexturePainter = function ( renderer, camera, mesh, src ) {
	...
	var initialize = function( src ) {
		...
		scope.texture = new Texture( scope.canvas, UVMapping, MirroredRepeatWrapping, MirroredRepeatWrapping );
		...
		scope.ortho = new OrthographicCamera( frustumSize * aspect / - 2, frustumSize * aspect / 2, frustumSize / 2, frustumSize / - 2, 0, 10 );
		scope.ortho.position.z = 50;
		scope.ortho.lookAt( scope.scene.position );

		var cursorTexture = new Texture( undefined, UVMapping, MirroredRepeatWrapping, MirroredRepeatWrapping );
		var cursorMaterial = new MeshBasicMaterial( { map: cursorTexture, transparent: true } );
		var cursorGeometry = new PlaneGeometry( cursorSize, cursorSize, 1, 1 );

		scope.cursor = new Mesh( cursorGeometry, cursorMaterial );
	...
	function faceIntersectsClip( clip, face ) {
	 ...
	 if ( clip.intersectsTriangle( new Triangle( vA, vB, vC ) ) )  return true;
	 return false;
	...
	function getDrawLocations () {
		...
		var vB = new Vector3();
		var vC = new Vector3();
		var origin = new Vector3().setFromMatrixPosition( scope.camera.matrixWorld );
		...
		// set clip-space.
		var min = new Vector3( - cursorUnits, - cursorUnits*aspect, - 0.1 );
		var max = new Vector3( + cursorUnits, + cursorUnits*aspect, + scope.camera.far );
		var clip = new Box3( min, max );
		...
		for ( var i = 0; i < intersects.length; i++ ) {
			...
			var plane = new Plane().setFromNormalAndCoplanarPoint( face.normal, vA );
			...
			for ( var v = 0; v < directions.length; v++ ) {
				...
				point = ray.intersectPlane( plane, new Vector3() );

				// bruch center in uv texture-space.
				// var uv = Triangle.getUV( point, vA, vB, vC, uvclip[0], uvclip[1], uvclip[2], new Vector2() );
				var uv = Triangle.getInterpolation ( point, vA, vB, vC, uvclip[0], uvclip[1], uvclip[2], new Vector2() );
